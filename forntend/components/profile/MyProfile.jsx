...
import { Button, Descriptions, Image, Modal, Result, Skeleton, Tag, Tooltip, Upload } from 'antd';
...
function MyProfile() {
  ...
  return (
    <>
        ...
        {error ? (
          ...
          <Descriptions
            ...
            extra={
              <>
                {!user?.verified && (
                  <Button style={{ marginTop: '10px', marginRight: '20px' }} onClick={handleVerifyEmail} shape='default' type='primary' size='large'>
                    Verify Email
                  </Button>
                )}

                <Button style={{ marginTop: '10px', marginRight: '20px' }} onClick={() => setEditProfileModal(true)} shape='default' type='primary' size='large'>
                  Edit Profile
                </Button>
              </>
            }
          >
            <Descriptions.Item label='Avatar' span={3}>
              {response?.data?.avatar ? <Image style={{ width: '100px', height: '100px' }} src={response?.data?.avatar} crossOrigin='anonymous' alt='user-image' /> : 'N/A'}
            ...
            <Descriptions.Item label='Full Name'>{response?.data?.fullName}</Descriptions.Item>
            <Descriptions.Item label='User Name' span={2}>
              {response?.data?.userName}
            </Descriptions.Item>
            <Descriptions.Item label='Email'>{response?.data?.email}</Descriptions.Item>
            <Descriptions.Item label='Phone' span={2}>
              {response?.data?.phone}
            </Descriptions.Item>

            <Descriptions.Item label='Role'>
              <Tag style={{ width: '60px', textAlign: 'center', textTransform: 'capitalize' }} color={response?.data?.role === 'admin' ? 'magenta' : 'purple'}>
                {response?.data?.role}
              </Tag>
            </Descriptions.Item>
            <Descriptions.Item label='Status' span={2}>
              <Tag style={{ width: '70px', textAlign: 'center', textTransform: 'capitalize' }} color={userStatusAsResponse(response?.data?.status).color}>
                {userStatusAsResponse(response?.data?.status).level}
              </Tag>
            </Descriptions.Item>
            <Descriptions.Item label='Verified'>
              <Tag style={{ width: '50px', textAlign: 'center', textTransform: 'capitalize' }} color={response?.data?.verified ? 'success' : 'error'}>
            ...
            <Descriptions.Item label='User Last Update Date'>{response?.data?.updatedAt?.split('T')[0]}</Descriptions.Item>
      ...
      {editProfileModal && <ProfileEditModal editProfileModal={editProfileModal} setEditProfileModal={setEditProfileModal} />}
    </>
  );
}
