...
function OrderPlaceModal({ bookingModal, setBookingModal }) {
  ...
  return (
    <Modal
      ...
      onOk={() => setBookingModal((prevState) => ({ ...prevState, open: false, roomId: null }))}
      ...
      footer={[
        <div key='custom-footer'>
          ...
          <Button onClick={() => setBookingModal((prevState) => ({ ...prevState, open: false, roomId: null }))} type='default' size='middle'>
            Cancel
          </Button>
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <Calendar
          style={{ width: '100%' }}
          plugins={[<DatePickerHeader key='date-picker-header' position='top' size='medium' />, <DatePanel style={{ width: '100%' }} key='date-panel' position='right' sort='date' />, <Toolbar key='toolbar' position='bottom' />]}
