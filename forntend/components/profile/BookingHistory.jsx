...
import { Button, Modal, Rate, Result, Space, Table, Tag, Tooltip } from 'antd';
    ...
    <>
      {error ? (
        <Result
          ...
          status='500'
          extra={
            <Button className='gradient-primary-btn' onClick={() => setFetchAgain(!fetchAgain)} type='primary' size='large' loading={loading} disabled={loading}>
              Try to Again
            </Button>
          }
        />
        ...
        <Table
          ...
          columns={[
            {
              ...
              title: (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  Booking Dates
                  {filter.sort === 'asce' ? <SortAscendingOutlined /> : <SortDescendingOutlined />}
                </div>
              ),
              dataIndex: 'booking_dates',
              render: (data) => arrayToCommaSeparatedText(data?.map((date) => date.split('T')[0])),
              sorter: true,
            ...
            },
            {
              ...
              render: (data) => (
                <Link href={`/rooms/${data?.room_slug}`} target='_blank'>
                  <Button className='btn-primary' size='large' type='link'>
                    {data?.room_name || 'N/A'}
                  </Button>
                </Link>
              ),
            ...
            },
            {
              ...
              render: (data) => <Tag color={bookingStatusAsResponse(data).color}>{bookingStatusAsResponse(data).level}</Tag>,
              align: 'center',
            },
            {
              key: 4,
              ...
              dataIndex: 'reviews',
              render: (data) => (
                <Tooltip title={data?.message} placement='top' trigger='hover'>
                  <span>{data ? <Rate value={data?.rating} disabled /> : 'N/A'}</span>
                </Tooltip>
              ),
              align: 'center',
            },
              ...
              render: (_, record) => (
                <Space size='middle'>
                  {record?.booking_status === 'pending' && (
                    <Button className='w-[85px]' type='default' size='middle' danger onClick={() => handleCancelBookingOrder(record?.id)}>
                      Cancel Booking
                    </Button>
                  )}

                  {record?.booking_status === 'in-reviews' && (
                    <Button className='w-[85px]' type='primary' size='middle' onClick={() => setAddReviewModal((prevState) => ({ ...prevState, open: true, bookingId: record?.id }))}>
                      To Reviews
                    </Button>
                  )}
          ...
          onChange={(_, __, sorter) => setFilter((prevSate) => ({ ...prevSate, sort: sorter?.order || 'asce' }))}
      ...
      {addReviewModal?.open && <ReviewAddModal addReviewModal={addReviewModal} setAddReviewModal={setAddReviewModal} setFetchAgain={setFetchAgain} />}
    </>
  );
}
